...
class AvaliacaoController extends Controller
{
    public function index()
    {
        ...
        $avaliacoes = DB::table('avaliacoes')
            ->join('funcionarios', 'avaliacoes.funcionario_id', '=', 'funcionarios.id')
            ->select('avaliacoes.*', 'funcionarios.nome', 'funcionarios.cargo', 'funcionarios.iniciais')
            ->orderBy('avaliacoes.data_avaliacao', 'asc')
            ->get();

        return view('avaliacoes', compact('totalPendentes', 'concluidasEsteMes', 'mediaGlobal', 'funcionarios', 'avaliacoes'));
    }
    ...
    public function store(Request $request)
    {
        ...
        return redirect()->back()->with('success', 'Avaliação agendada com sucesso!');
    }
    ...
    public function update(Request $request, $id)
    {
        ...
        DB::table('avaliacoes')->where('id', $id)->update([
            'data_avaliacao' => $request->data_avaliacao,
            'nota'           => $request->nota,
            'estado'         => $estado,
