...
<div class="d-flex justify-content-between align-items-start mb-4">
    <div>
        <h1 class="font-serif display-6 fw-normal mb-1">Editar Estatística</h1>
        <p class="text-accent fw-normal mb-0" style="font-size: 15px;">Alterar os dados da estatística</p>
    </div>
...
<div class="card-custom shadow-sm bg-white p-4" style="max-width: 760px;">
    <form action="{{ route('admin.estatisticas.update', $estatistica) }}" method="POST">
        @csrf
        @method('PUT')

        <div class="mb-3">
            <label class="form-label fw-medium">Valor <span class="text-danger">*</span></label>
            <input type="text" name="value" class="form-control" value="{{ old('value', $estatistica->value) }}" placeholder="Ex.: 10" required>
        </div>

        <div class="mb-3">
            <label class="form-label fw-medium">Sufixo</label>
            <input type="text" name="suffix" class="form-control" value="{{ old('suffix', $estatistica->suffix) }}" placeholder="Ex.: +">
        ...
        <div class="mb-3">
            ...
            <input type="text" name="label" class="form-control" value="{{ old('label', $estatistica->label) }}" placeholder="Ex.: anos de experiência" required>
        </div>

        <div class="mb-3">
            <label class="form-label fw-medium">Ordem de apresentação</label>
            <input type="number" name="sort_order" class="form-control" value="{{ old('sort_order', $estatistica->sort_order) }}" min="0">
