...
<div class="d-flex justify-content-between align-items-start mb-4">
    <div>
        <h1 class="font-serif display-6 fw-normal mb-1">Editar Serviço</h1>
        <p class="text-accent fw-normal mb-0" style="font-size: 15px;">Alterar os dados do serviço</p>
    </div>
...
<div class="card-custom shadow-sm bg-white p-4" style="max-width: 760px;">
    <form action="{{ route('admin.servicos.update', $servico) }}" method="POST">
        ...
        <div class="mb-3">
            ...
            <input type="text" name="title" class="form-control" value="{{ old('title', $servico->title) }}" required>
        </div>

        <div class="mb-3">
            <label class="form-label fw-medium">Descrição <span class="text-danger">*</span></label>
            <textarea name="description" class="form-control" rows="3" required>{{ old('description', $servico->description) }}</textarea>
        </div>

        <div class="mb-3">
            <label class="form-label fw-medium">Ícone</label>
            <input type="text" name="icon" class="form-control" value="{{ old('icon', $servico->icon) }}" placeholder="fa-solid fa-briefcase">
            <small class="text-muted">Classe FontAwesome (ex.: <code>fa-solid fa-chart-line</code>) ou código SVG completo.</small>
        </div>

        <div class="mb-3 form-check">
            <input type="checkbox" class="form-check-input" id="is_featured" name="is_featured" value="1" {{ old('is_featured', $servico->is_featured) ? 'checked' : '' }}>
